...
def convert_to_isl(text):
    ...
    for text in result:
        ...
        video_folder = "E:/1_BTech/Major_Project/FINAL/Text_to_ISL/islVideos2"
        found = False

        # Loop through all video files in the folder
        for video_file in os.listdir(video_folder):
            # Check if the text[0] (the word) is present in the filename (case-insensitive and partial match)
            if text[0].lower() in video_file.lower():
                ...
                found = True
                break  # Break out of the loop once we find the first match
